      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
        ...
        <div className="md:col-span-8 space-y-8">
          ...
          {profile.education.map((edu, idx) => (
            <div
              key={idx}
              className="p-6 rounded border border-border bg-surface space-y-4 shadow-sm"
            >
          ...
          <div className="pt-2 space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {profile.interests.map((topic, idx) => (
                <span
                  key={idx}
                  className="text-xs font-mono px-3 py-1 rounded bg-surface border border-border text-foreground hover:border-neutral-400 transition-colors"
                >
                  {topic}
                </span>
              ))}
